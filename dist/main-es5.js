...
(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["main"], {
      ...
      _createClass(SitesComponent, [{
        ...
        value: function exportStatus(id) {
          ...
          this.bspServiceService.getSiteOnlineStatus(id).toPromise().then(function (result) {
            ...
            _this7.loaderService.dismiss(); //this.helperService.openSomethingWrongDialog(error);
        ...
        value: function generateXMLFile(siteName) {
          ...
          this.bspServiceService.generateXMLForSite(siteName).toPromise().then(function (res) {
            ...
            for (var i = 0; i < lenght; i++) {
              if (_this8.Sites[i].siteName == siteName) {
                //this.Sites[i].generateFile = result.result;
                _this8.Sites[i].generateFile = result.result;
              }
            }
          })["catch"](function (error) {
        ...
        value: function exportFile(attributes) {
          ...
          if (result["result"] == "SUCCESS") {
            this.bspServiceService.exportFileToSite(computerName, result.filename, dirname).toPromise().then(function (res) {
              var lenght = _this9.Sites.length;

              for (var i = 0; i < lenght; i++) {
                if (_this9.Sites[i].customAttributeSets[0].attributes[3].value == computerName) {
                  _this9.Sites[i].ExportFile = res.result;
                }
              }
            })["catch"](function (error) {
      ...
      _createClass(UpdateItemsDataComponent, [{
        ...
        value: function GetItemsForTheCat(catId) {
          ...
          this.bspServiceService.getCatItemDetails(catId).toPromise().then(function (itemModel) {
            ...
            _this10.items = itemModel;
            _this10.catId = catId;

            _this10.stateService.setSelectedCat(_this10.catId);

            _this10.loaderService.dismiss(); // if (itemModel.items.length > 0) {
      ...
      _createClass(UserMgmtComponent, [{
        ...
        value: function getRetailerUsers() {
          ...
          this.accountService.getUsers().toPromise().then(function (users) {
            ...
            _this11.users = users.sort(function (u1, u2) {
              return u1.name > u2.name ? 1 : -1;
            });

            _this11.loaderService.dismiss();
          })["catch"](function (error) {
            _this11.loaderService.dismiss();

            _this11.helperService.openSomethingWrongDialog(error);
          });
        }
      }, {
        ...
        value: function signIn() {
          ...
          try {
            ...
            this.accountService.signIn(this.formModel.value).toPromise().then(function (response) {
              ...
              _this16.stateService.setAccountModel(response.user);

              _this16.stateService.setAuthorities(response.user.role.roleName);

              _this16.stateService.setToken(response.token);

              _this16.isSignInExecuting = false;

              _this16.goToHomepage();
            })["catch"](function (error) {
              console.log("Token :" + error);

              _this16.loaderService.dismiss();

              _this16.isSignInExecuting = false;

              _this16.helperService.openSomethingWrongDialog(error);
            });
          } catch (error) {
  ...
  function srcEnvironmentsEnvironmentTs(module, __webpack_exports__, __webpack_require__) {
    ...
    var environment = {
      production: true,
      backendUrl: 'https://mobile-pos-demo.azurewebsites.net/api' //backendUrl: 'http://localhost:8080/api'
